...
namespace TileStories.Tests
{
    ...
    public class MarkerOverlapResolverTests
    {
        ...
        public IEnumerator MarkerBillboard_CameraMoved_MarkerRotationMatchesCamera()
        {
            ...
            // with the 180-degree yaw offset that MarkerBillboard applies
            var expectedRotation = cam.transform.rotation * Quaternion.Euler(0, 180, 0);
            float angleDiff = Quaternion.Angle(markerGO.transform.rotation, expectedRotation);
            Assert.Less(angleDiff, 0.1f,
                $"Marker rotation should match camera rotation (with 180 yaw offset). Angle difference: {angleDiff:F3} degrees.");
