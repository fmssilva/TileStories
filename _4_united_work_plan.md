...
### Stage 0 — UX Design Sprint (Week 0 — before any code is written)
...
`Framework/Tests` folder — unit tests for the Stage 4 validator/compiler specifically
(easy to test in isolation, and the place where a silent bug would do the most damage
across all three walls at once) are worth the investment; UI/gameplay-level testing
stays informal (the field-testing protocols already specified per stage) rather than
trying to automate AR-camera-dependent behaviour, which has a poor cost-to-value ratio
for a six-month solo thesis.
