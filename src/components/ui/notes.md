documented. However, I do have some suggestions for improvements:
Issues to Fix:
1. Variant Inheritance Problem
The TabsTrigger doesn't automatically inherit the variant from TabsList. Users need to repeat it:
tsx// Current: redundant variant specification
<TabsList variant="pills">
  <TabsTrigger variant="pills" value="tab1">Tab 1</TabsTrigger>
  <TabsTrigger variant="pills" value="tab2">Tab 2</TabsTrigger>
</TabsList>
2. Missing Animation
TabsContent lacks the smooth transition you'd expect.
3. Color Scheme + Appearance Conflicts
Some combinations don't work well (e.g., colorScheme with pills variant which already sets background).
Improved Version:
typescript/**
 ...
 * - Automatic variant inheritance from TabsList to TabsTrigger
 * 
 * BASIC USAGE:
 * ```tsx
    ...
    ],
    {
        variants: {
            ...
            variant: {
                ...
                line: [
                    'px-4 py-2 border-b-2 border-transparent -mb-[2px]',
...
interface TabsListProps
     ...
     */
    zIndex?: number;
    /** Appearance style (inherited by TabsTrigger children) */
    appearance?: VariantProps<typeof tabsTriggerVariants>['appearance'];
    /** Color scheme (inherited by TabsTrigger children) */
    colorScheme?: VariantProps<typeof tabsTriggerVariants>['colorScheme'];
}
...
const TabsTrigger = React.forwardRef
    ...
    TabsTriggerProps
>(({ className, variant, size, colorScheme, appearance, ...props }, ref) => {
...
✅ Context-based variant inheritance - Set variant once on TabsList, automatically applies to all TabsTrigger children
✅ Smooth animations - Added fade-in/slide-in transitions to TabsContent
✅ Better line variant - Fixed border alignment with -mb-[2px]
✅ Default text colors - Inactive tabs now have proper muted colors
✅ Compound variants - Pills variant properly ignores colorScheme since it uses background colors
✅ Better button variant - Added background colors for better contrast
✅ Memoized context - Performance optimization for context value

Usage Now:
tsx// ✨ No more repetition!
<Tabs defaultValue="overview">
  <TabsList variant="pills" size="spacious" appearance="3d">
    <TabsTrigger value="overview">Overview</TabsTrigger>
    <TabsTrigger value="analytics">Analytics</TabsTrigger>
...
// Still allows per-trigger overrides if needed
<TabsList variant="line" colorScheme="blue">
  <TabsTrigger value="tab1">Normal</TabsTrigger>
  <TabsTrigger value="tab2" appearance="3d">This one is 3D!</TabsTrigger>
</TabsList>
This is