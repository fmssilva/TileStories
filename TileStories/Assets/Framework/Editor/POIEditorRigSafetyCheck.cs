...
{
    // Intercepts Play Mode entry and scene saving when POIEditorRig still
    // has child objects. For Play Mode, delegates to
    // POIEditorToolWindow.PromptBeforePlayOrBuild which shows an interactive
    ...
    // Build-time checks live in POIEditorRigBuildCheck.
    [InitializeOnLoad]
    public static class POIEditorRigSafetyCheck
    {
        static POIEditorRigSafetyCheck()
        {
        ...
        private static void OnSceneSaving(Scene scene, string path)
        {
            ...
            {
                Debug.LogWarning(
                    $"[POIEditorRigSafetyCheck] POIEditorRig still has " +
        ...
        private static void OnPlayModeStateChanged(PlayModeStateChange state)
        {
            ...
            if (!POIEditorToolWindow.PromptBeforePlayOrBuild(isBuild: false))
            {
