...
namespace TileStories.Editor
{
    public partial class POIEditorToolWindow
    {
        ...
        private void SaveConfig()
        {
            if (_config == null)
            {
                Debug.LogWarning("[POIEditor] Nothing to save.");
                return;
            }
            ...
            Debug.Log($"[POIEditor] Saved config to {_configPath}");
        }
        ...
        private void LoadConfig()
        {
            ...
            {
                Debug.LogError($"[POIEditor] Config not found at {_configPath}");
                return;
            }
            ...
            if (_config == null)
            {
                Debug.LogError("[POIEditor] Failed to parse config.");
                return;
            }
        ...
        private void CopyToStreamingAssets()
        {
            if (!File.Exists(_configPath))
            {
                Debug.LogError("[POIEditor] Source config not found.");
                return;
            }
            ...
            AssetDatabase.Refresh();
            Debug.Log($"[POIEditor] Copied to StreamingAssets: {_streamingConfigPath}");
        }
    }
}
