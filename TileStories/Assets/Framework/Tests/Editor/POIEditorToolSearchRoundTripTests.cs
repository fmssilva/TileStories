...
{
    // Tier 0 EditMode tests for the Block 5 Phase 5.1 Search & Filter editor
    ...
    public class POIEditorToolSearchRoundTripTests
    {
        [Test]
        ...
        public void FoldoutSectionMethods_AndState_Exist()
        {
            ...
            Assert.IsNotNull(
                ...
                "DrawGlobalSearchFilterSection must be wired on the editor window");

            Assert.IsNotNull(
                t.GetMethod("DrawPoiSearchKeywordsField",
                    BindingFlags.NonPublic | BindingFlags.Instance),
                "DrawPoiSearchKeywordsField must be wired on the editor window");
        ...
        {
            var window = EditorWindow.CreateInstance<POIEditorToolWindow>();
            var config = new WallConfigData
            {
