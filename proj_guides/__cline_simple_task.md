...
# `load + populate rig at same time?`
currently we have 2 buttons for "load" and then for "populate rig". But if a developer clicks on load, i think that for sure he wants to populate rig and see the data he has on the config file right away in the scene yes? so should we combine "load + populate rig" in the "load button click"? 
and should we delete the populate rig button, or that can stay the same because we might click in clear rig and so then we want to populate again?? ?? do a check of those buttons we have...

check the domain guide file of this whole editor tab: 
`C:\Users\franc\Desktop\TileStories\proj_guides\_5.1_Editor_Tab.md`


# `TEACHER ROLE - LEARNING SUMMARIES`
