...
Moved to the guidelines `10_structure.md`. Read that file for the organizing
principles and the full, current file tree. This file (`_0_work_plan.md`)
stays focused on the stage-by-stage roadmap: what to build next and why,
not where every file lives. A local implementing agent working on one
concrete task should read only the `10_structure.md` file, and the concrete `domain_X` file, not this global project work plan file. 
This file is for planning new work, not onboarding onto the
existing structure.
...
substantially satisfy this stage too: an `EditorWindow`, a "Populate Rig
from JSON" button (== this stage's "Populate Scene Markers"), Save/Copy-to-
StreamingAssets, category/badge/outline taxonomy tables with inline help
text, and every Wizard feature already optional (hand-editing `config.json`
directly still works, nothing is gated behind the tool). **See
`_5.1_Editor_Tab.md` for the current, authoritative state of this tool** —
don't replan it from scratch here. What's genuinely still missing relative
to this stage's original scope: the guided "minimal viable wall" first-run
flow described below (today's tool assumes an existing `config.json` to
load, it doesn't walk a developer through creating one from nothing), and
formal `[InitializeOnLoad]` auto-open on first import. Treat the rest of
this section as background on *why* the tool looks the way it does, not as
