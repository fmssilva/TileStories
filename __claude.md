...
- `Attach: global work plan, guidelines, current project zip, and any existing prototypes/docs for this domain if they exist.`

Read the attached documents with full work plan for a project i want to implement: `_0_work_plan.md`. Read all lines of the file so you have a clear picture. No exceptions.

Also read the general guidelines I want to follow when implementing the project. Read also the full document.

Read them fully, no exceptions.

And now we're starting `Domain X`. For this whole conversation, don't touch the project zip or write implementation code — we're only discussing and planning. Give me your read of what the work plan already says about `Domain X`, and what's actually built vs. just planned, before we design anything.

Read fully. Planning only, no code. Tell me what's already said about this domain and what's actually built vs. planned.

Also tell me:
- Does this domain have a visual/rendered component (UI, AR markers, effects, animations)? If so, flag that now — it changes how we should plan Phase A/B testing later (guidelines §6.4).
- Does this domain have, or will it need, an Editor-time authoring/preview tool (a custom `EditorWindow`, custom inspector, etc.) that touches this domain's real runtime objects outside Play Mode? If so, flag that too — guidelines §6.4.1 (Edit-Mode tooling parity) applies and needs planning alongside Phase A/B. This is a different axis from Phase A/B, not a third phase after it — say explicitly whether it does or doesn't apply here, don't leave it implicit.

Be good, not fast.

# 1.1 **Continuing Kickoff in a new Chat**

*(Scratch example for the current domain in progress — not a general template
step; safe to delete once this domain is done.)*

And so now we will continue planning the impelemntation of the `domain about the poi marker design`. I send attached the current plan and status of this domain implementation we have currently. read all the lines of this file so you get up to date about the current status. And then also check the current actual code in the zip attached (don't trust in the work plan guide because it might be out of sync with the actuall implemented folders and files and code).
...
Phase A — isolated test scene, zero AR/tracking/config.json:
  - A dedicated, non-shipping test scene (excluded from Build Settings) that exercises every rendering variant of this domain directly, bypassing whatever
    real data pipeline (config.json, WallSession, etc.) normally drives it.
  - Built and verified incrementally: start with the single simplest case, confirm
    it's correct (automated structural check + one human glance), only then add
    the next variant — never build every variant first and debug all at once.
  - Automated structural assertions (non-null references, correct component
    enabled/disabled states, correct resolved values) runnable via Unity MCP with
    no screenshots needed, PLUS one human visual pass at the end.
  - Do not proceed to Phase B until every variant in Phase A is confirmed both
    ways.

Phase B — real pipeline integration:
  - Wire the now-proven-correct system into the real domain flow (AR tracking,
    config.json, whatever this domain's actual production path is).
  - If Phase B shows a problem that Phase A didn't: that's a strong signal the bug
    is in the integration/config layer, not the rendering system itself — Phase A
    already proved that part correct in isolation. Debug accordingly, don't
